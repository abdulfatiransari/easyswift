...
export default function LandingPage() {
    ...
    const claimFunds = async (index, receiver) => {
        try {
            ...
            if (wallet.toLowerCase() === receiver.toLowerCase()) {
                ...
                const lockboxkey = await contract.methods.getLockBoxKeyAtIndex(Number(index)).call();
                const sendAmount = await contract.methods
    ...
    const reclaimFunds = async (index, creator) => {
        try {
            ...
            if (wallet.toLowerCase() === creator.toLowerCase()) {
                const lockboxkey = await contract.methods.getLockBoxKeyAtIndex(Number(index)).call();
                const reclaimAmount = await contract.methods.reclaimFunds(lockboxkey).send({ from: wallet });
            }
        } catch (error) {
            console.log(error);
        }
    ...
    const withdraw = async () => {
        try {
            ...
            const withdrawAmount = await contract.methods.withdraw(Number(checkBalance)).send({ from: wallet });
        } catch (error) {
            console.log(error);
        }
    };
    ...
    const withdrawData = async () => {
        ...
        try {
            const { contract } = web3Obj;
            for (var i of userDetails) {
                const checkBalance = await contract.methods.getBalance(i.receiver).call();
                let balance = Number(checkBalance);
                arr.push(balance);
    ...
    const withdrawfees = async () => {
        try {
            ...
            if (owner.toLowerCase()) {
                const totalBalance = await contract.methods.getCollectedFeeAmount().call({ from: wallet });
                if (totalBalance > 0) {
                    const withdraw = await contract.methods.withdrawFees().send({ from: wallet });
                } else {
    ...
    const toggle = async () => {
        try {
            ...
            if (wallet.toLowerCase() === owner.toLowerCase()) {
                const toggle = await contract.methods.toggleContractActive().send({ from: wallet });
            } else {
    ...
    const setNewOwner = async () => {
        try {
            const { contract } = web3Obj;
            const new_owner = await contract.methods.setOwner(value2.address).send({ from: wallet });
            setOwner(new_owner);
        } catch (error) {
            console.log(error);
        }
    ...
    const set_OwnerFee = async () => {
        try {
            ...
            const _ownerfee = await contract.methods.setOwnerFee(weitoeth).send({ from: wallet });
            setOwnerFee(weitoeth);
        } catch (error) {
            console.log(error);
        }
    ...
    const set_deadline = async () => {
        try {
            ...
            const set_dead_line = await contract.methods.setDeadline(convert).send({ from: wallet });
            ...
            <div className="px-[20px]">
                ...
                <div className="flex items-center bg-[#222] border border-[#080808] rounded-[4px] mb-[20px] min-h-[50px] justify-between px-[15px] py-[10px]">
                    ...
                    <div className="flex items-center gap-x-2">
                        <div>
                            <p className="text-white">{user.displayType || ""}</p>
                        </div>
                        <div>
                            {wallet === "" ? (
                ...
                ${!wallet.toLowerCase() ? "hidden" : wallet.toLowerCase() === owner.toLowerCase() ? "flex" : "hidden"}
                ...
                <div className="flex justify-between">
                    <div className="flex p-[15px] border border-[#ddd] min-w-[218px] rounded-[4px]">
                        <div className="flex flex-col">
                            ...
                            <p className="text-[36px] mt-[20px] mb-[10px]">{`${contractBalance}` || "0"} ETH</p>
                        </div>
                    </div>
                    <div className="flex p-[15px] border border-[#ddd] min-w-[218px] rounded-[4px]">
                        <div className="flex flex-col">
                            <p className="text-[14px] my-[10px]">OwnerFees</p>
                            <p className="text-[36px] mt-[20px] mb-[10px]">{`${ownerFee}`} ETH</p>
                        </div>
                    </div>
                    <div className="flex p-[15px] border border-[#ddd] min-w-[218px] rounded-[4px]">
                        <div className="flex flex-col">
                    ...
                    <div className="flex items-center rounded-[4px]">
                        ...
                        <div className="flex border border-[#ccc] px-[16px] py-[10px] w-full">
                            <input
                                ...
                                onChange={(event) => setValue({ ...value, reciever: event.target.value })}
                                className="w-full outline-none"
                            />
                        </div>
                    </div>
                    <div className="flex items-center rounded-[4px]">
                        ...
                        <div className="flex border border-[#ccc] px-[16px] py-[10px] w-full">
                            <input
                                ...
                                type="password"
                                value={value.pass1}
                                onChange={(event) => setValue({ ...value, pass1: event.target.value })}
                                className="w-full outline-none"
                            />
                        </div>
                    </div>
                    <div className="flex items-center rounded-[4px]">
                        ...
                        <div className="flex border border-[#ccc] px-[16px] py-[10px] w-full">
                            <input
                                ...
                                type="password"
                                value={value.pass2}
                                onChange={(event) => setValue({ ...value, pass2: event.target.value })}
                                className="w-full outline-none"
                            />
                        </div>
                    </div>
                            ...
                            {userDetails
                                ...
                                .map((item, idx) => (
                                    <tr key={idx} className="text-center">
                                        <td>
                                            <h4>
                                                {`${item.creator.slice(0, 8)}...${String(item.creator).slice(-5)}`}
                                                <span></span>
                                            </h4>
                                        </td>
                                        <td>
                                            <h4>
                                                {`${item.receiver.slice(0, 8)}...${String(item.receiver).slice(-5)}`}
                                                <span></span>
                                            </h4>
                                        </td>
                                        ...
                                        <td>
                                            ...
                                            {wallet.toLowerCase() === item.creator.toLowerCase() && item.amount && (
                                                <button onClick={() => reclaimFunds(item.index, item.creator)}>
                                                    Sender reclaim
                                                </button>
                                            )}
                                        </td>
                        ...
                        <div className="flex flex-col bg-[#ddd] rounded-[20px] gap-y-2 min-w-[400px] p-5 ">
                            ...
                            <div className="flex gap-x-2 items-center">
                                ...
                                <input
                                    ...
                                    type="password"
                                    value={value1.pass1}
                                    onChange={(event) => setValue1({ ...value1, pass1: event.target.value })}
                                />
                            </div>
                            <div className="flex gap-x-2 items-center">
                                <FaLock />
                                <input
                                    ...
                                    type="password"
                                    value={value1.pass2}
                                    onChange={(event) => setValue1({ ...value1, pass2: event.target.value })}
                                />
                            </div>
                ...
                {settting && (
                    <div className="flex w-screen justify-center absolute top-20">
                        <div className="flex flex-col bg-[#ddd] rounded-[20px] gap-y-2 min-w-[400px] p-5 ">
                            ...
                            <div>
                                <p>Set New Owner</p>
                                <p>Current Owner: {owner.toLowerCase()}</p>
                                <div className="flex gap-x-2 items-center">
                                    <CgProfile />
                                    <input
                                        placeholder="new owner"
                                        // value={value2.address}
                                        onChange={(event) => setValue2({ ...value2, address: event.target.value })}
                                    />
                                    <button
                                        onClick={() => {
                                            setNewOwner();
                            ...
                            <div>
                                ...
                                <div className="flex gap-x-2 items-center">
                                    ...
                                    <input
                                        ...
                                        onChange={(event) => setValue2({ ...value2, ownerfee: event.target.value })}
                                    />
                                    <button
                                        onClick={() => {
                                            set_OwnerFee();
                            ...
                            <div>
                                ...
                                <div className="flex gap-x-2 items-center">
                                    ...
                                    <input
                                        ...
                                        onChange={(event) => setValue2({ ...value2, _deadline: event.target.value })}
                                    />
                                    <button
                                        onClick={() => {
                                            set_deadline();
